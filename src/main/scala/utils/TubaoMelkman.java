 ...
 * @author zl
 */
public class TubaoMelkman {
    private Point[] pointArray;//坐标数组
    private final int N;//数据个数
    private int D[]; // 数组索引

    public TubaoMelkman(List<Point> pList) {
...
//                    + pointArray[D[i]].getY());
    ...
    public static void main(String args[]) {
        ...
        while (str != null) {
            ...
            p.setX(x);
            p.setY(y);
            p.setDeviceId(Integer.parseInt(s[2].trim()));
        ...
        TubaoMelkman m = new TubaoMelkman(pointList);
        Point[] tubaoPoint = m.getTubaoPoint();
        for (Point point : tubaoPoint) {
            if (point != null) {
                System.out.println(point.getX() + "," + point.getY() + "," + point.getDeviceId() +"," + point.getArCos());
            }
        }
    }
}
